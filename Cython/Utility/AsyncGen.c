...
__Pyx_async_gen_init_hooks(__pyx_PyAsyncGenObject *o)
{
...
#if CYTHON_COMPILING_IN_LIMITED_API
    {
        ...
        firstiter = PyTuple_GetItem(async_gen_hooks, 0);
        if (unlikely(!firstiter)) {
            Py_DECREF(async_gen_hooks);
            return 1;
        }
        if (firstiter == Py_None) firstiter = NULL;
        finalizer = PyTuple_GetItem(async_gen_hooks, 1);
        Py_DECREF(async_gen_hooks);
        if (unlikely(!finalizer)) {
            return 1;
        }
        if (finalizer == Py_None) finalizer = NULL;
    }
#endif
