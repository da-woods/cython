...
try:
    from dataclasses import (
        MISSING, _DataclassParams, Field, dataclass, 
        _HAS_DEFAULT_FACTORY, _FIELD, _FIELD_INITVAR, _FIELD_CLASSVAR,
        field as _field_func,
    )
except ImportError:
    # Fallback code
    ...
    _missing_field_args = []
    if "kw_only" not in _field_func.__kwdefaults__:
        _missing_field_args.append("kw_only")
    if _missing_field_args:
        def field(*args, **kwds):
            for missing in _missing_field_args:
                kwds.pop(missing, None)
            return _field_func(*args, **kwds)
    else:
        field = _field_func